...
import { Send, Phone, MessageCircle, RefreshCw, ChevronLeft } from "lucide-react";
      ...
      <div className={`w-full md:w-80 border-r border-slate-100 flex flex-col shrink-0 ${activeChat ? "hidden md:flex" : "flex"}`}>
        <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
          <h2 className="font-extrabold text-slate-800 text-sm uppercase tracking-wider flex items-center gap-2">
            <MessageCircle size={16} /> Liên lạc ứng viên
          </h2>
          <button
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline">
                      <h4 className="font-bold text-slate-800 text-xs truncate">{c.name || "Sinh viên"}</h4>
      ...
      <div className={`flex-1 flex flex-col bg-slate-50/50 ${!activeChat ? "hidden md:flex justify-center items-center p-12 text-slate-400 text-center" : "flex"}`}>
        {!activeChat ? (
          <div className="flex flex-col items-center gap-2">
            ...
            <h3 className="font-extrabold text-slate-700 text-sm">Chưa chọn ứng viên trò chuyện</h3>
            <p className="text-xs text-slate-400 max-w-xs mt-1">Bấm chọn một bạn sinh viên bên trái để bắt đầu nhắn tin trao đổi công việc realtime.</p>
          </div>
        ) : (
          <>
            {/* Active Header */}
            <div className="p-4 bg-white border-b border-slate-100 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">Sinh viên ứng tuyển</p>
                </div>
              </div>
